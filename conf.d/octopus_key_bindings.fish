...
bind \cf\ci 'commandline -it (sqlite3 $HOME/.local/fsindex.db "SELECT path FROM fsindex" | fzf --preview-window="bottom:15:wrap" --preview=$FZF_PREVIEW_UNIVERSAL_CMD) 2>/dev/null; commandline -f repaint'
bind \e\\ 'commandline -it (sqlite3 $HOME/.local/fsindex.db "SELECT path FROM fsindex" | fzf --preview-window="bottom:15:wrap" --preview=$FZF_PREVIEW_UNIVERSAL_CMD) 2>/dev/null; commandline -f repaint'
