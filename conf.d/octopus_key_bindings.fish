...
bind -k btab 'commandline -it (sqlite3 $HOME/.local/fsindex.db "SELECT path FROM fsindex" | fzf) 2>/dev/null; commandline -f repaint'
bind \e\\ 'sqlite3 $HOME/.local/fsindex.db "SELECT path FROM fsindex" | fzf; commandline -f repaint'
