...
function init_env
    ...
    '-i '\
    '--header=\'alt-o: open | alt-s: subl | alt-t: tail | alt-l: less | alt-c: copy | alt-p: copy full path\' '\
    '--multi '\
    '--color fg:15,bg:-1,hl:11,fg+:15,bg+:12,hl+:0,info:3,prompt:3,spinner:7,pointer:3,marker:9 '\
    '--bind \'alt-o:execute(open {}),alt-s:execute(subl {}),alt-t:execute(tail -f -n 20 {}),alt-l:execute(less -f {}),alt-c:execute-silent(echo {} | pbcopy),alt-p:execute-silent(echo $PWD/{} | pbcopy)+abort\''
    end
