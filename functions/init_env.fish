...
function init_env
    if not set -q NO_FISH_INIT; and not set -q LC_NO_FISH_INIT
        ...
        '--bind \'f2:execute(kitty-tab {}),f3:execute-silent(qlmanage -p {} &),f4:execute(subl {}),f5:execute-silent(pcp {}),f6:execute-silent(qmv -f do -e sublw {} &),f7:execute(opendir {}),f8:execute(trash {}),f9:execute(alf {}),f10:execute(open {}),f11:execute(tail -F -n 100 {})+abort,f12:execute(bat --color always {})+abort\' '\
